...
public class TelegramBot extends TelegramLongPollingBot {
	...
	public void onUpdateReceived(Update update) {
		...
		;
		switch (messageText) {
			case "/start" -> message = CommandStart.init(chatId);
			case "Consultation" -> message = null; // there will be a consultation mod
			case "What happened?" -> message = CommandConsultation.init(chatId);
			case "Help of a psychotherapist" -> message = null; // there will be a PayPal service
			default -> message = CommandStart.getWrongCommand(chatId);
		}
